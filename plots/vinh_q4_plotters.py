...
def analyze_prefetch_sizes(benchmarks, sizes, results_dir):
    ...
    for benchmark in benchmarks:
        for size in sizes:
            file_name = f"{benchmark}.champsimtrace.xz-perceptron-vinh_stride_256_{size}-no-no-lru-1core.txt"
...
def plot_useful_counts(useful_counts, sizes):
    ...
    plt.title('L1 D-cache Useful Prefetches vs. Prefetch degree on 256')
    plt.grid(True, linestyle='--', alpha=0.7)
    plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
    
    plt.tight_layout()
    plt.savefig('prefetch_useful_counts_degree_log.png', bbox_inches='tight', dpi=300)
